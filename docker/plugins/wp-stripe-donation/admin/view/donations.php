...
if ( count( $wpsdDonations ) > 0 ) {
    foreach ( $wpsdDonations as $donation ) {
        ...
        ?></td>
                        <td><?php 
        printf( '%s', $donation->wpsd_donation_for );
        ?></td>
                        <td><?php 
        printf( '%s', $donation->wpsd_donator_name );
        ?></td>
                        <td><?php 
        printf( '%s', $donation->wpsd_donator_email );
        ?></td>
                        <td><?php 
        printf( '%s', date( 'M d, h:i A', strtotime( $donation->wpsd_donation_datetime ) ) );
        ?>
                        </td>
                    </tr>
                    <?php 
